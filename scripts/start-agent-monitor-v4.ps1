# Agent Team 完整监控服务 v4 - UTF-8 修复版
# 显式处理 UTF-8 编码，解决中文乱码
...
try {
    ...
    Write-Host "小妖的 Agent Team - 完整监控服务 v4" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "✅ 监控服务已启动（UTF-8 编码）" -ForegroundColor Green
    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            if ($path -eq "/api/sessions") {
                ...
                # 使用 UTF-8 编码序列化 JSON
                $jsonOptions = [System.Text.Encoding]::UTF8
                $responseData = @{
                    success = $true
                    sessions = $sessions
